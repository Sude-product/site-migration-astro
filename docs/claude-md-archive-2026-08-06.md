## Proje Durumu (son güncelleme: 2026-08-06)

**2026-08-06 günlüğü — Landing Page (reklam trafiği, Faz 6 öncelik #1)
BAŞLADI, keşif tamamlandı, kullanıcı onayı bekleyen 2 madde nedeniyle
DURAKLADI — kod değişikliği HENÜZ YOK.** Kullanıcı reklam trafiği için
tek sayfalık, çok adımlı formlu bir dönüşüm sayfası istedi (kapsam:
Anasayfadan az, Online Sunum Talebi'nden fazla içerik — kısa başlık +
3-4 madde fayda listesi + 2 adımlı form: Adım1 e-posta+ad-soyad, Adım2
telefon+şirket). Önce keşif yapıldı, kod yazılmadan 3 soru soruldu.

**Keşif bulguları (kullanıcının bazı varsayımları YANLIŞ çıktı):**
- **"Faz 2'de kurduğumuz SendGrid+D1 form backend'i" — BÖYLE BİR ALTYAPI
  YOK.** Repo'da `functions/` dizini, `wrangler.toml`, SendGrid/D1
  referansı SIFIR — `HeroForm.tsx`'in `handleSubmit`'i hâlâ yalnızca
  `console.log` (dosya başındaki `// TODO: backend entegrasyonu` yorumu
  ile tutarlı, CLAUDE.md'nin kendisi de bunu Açık nokta #2'de bekleyen
  Faz 2 işi olarak listeliyor). **Kullanıcı kararı: pilot için de aynı
  `console.log` stub deseni kullanılacak** — gerçek backend (Cloudflare
  Pages Functions + SendGrid + D1) AYRI, ayrıca kapsamlandırılacak bir
  iş olarak ele alınacak, bu turun kapsamına GİRMİYOR.
- **"Müşteriler sayfasında kırmızı elle-çizilmiş yıldız + konuşma
  balonu illüstrasyonu" — BU PROJEDE HİÇ MİGRATE EDİLMEMİŞ.** İkinci bir
  keşif turu (`CustomerStoriesPage.astro` + `musteriler.astro` +
  `src/assets`/`public` tam taranarak) böyle bir SVG/görsel/asset
  bulamadı — sayfanın hero'sunda yalnızca `<h1>`+açıklama+"Hemen Başvur"
  butonu (sol) + düz fotoğraf `<img>` (sağ) var, dekoratif yıldız/balon
  YOK. Muhtemelen yalnızca canlı idenfit.com kaynağında var olan, bu
  migrasyona hiç taşınmamış bir öğe. **Kullanıcı fallback'i kabul etti:**
  bunun yerine landing page'in dekoratif panelinde İK Dijital Olgunluk
  Testi hero'sundaki kırmızı Lottie glow/arc animasyonu (`lottie-web`,
  `public/lottie/hr-maturity-arc.json`, `HrMaturityTest.tsx`'teki
  `MaturityArcDecoration` component'i) referans alınacak.
- **Yeniden kullanılabilir doğrulanmış parçalar:** `HeroForm.tsx`'in
  `variant="presentation"` modu (Online Sunum Talebi'nin kullandığı,
  pill input/yeşil buton/büyük-harf placeholder stili) — ama bu TEK
  ADIMLIK bir form, landing page'in 2 adımlı akışı için alanları/
  `PhoneCountrySelect.tsx`'i yeniden kullanan ama adım state'ini SARAN
  yeni bir wrapper gerekiyor (`HeroForm`'un kendi markup'ı olduğu gibi
  kullanılamaz). `BaseLayout.astro`'da `noindex`/robots meta için HİÇ
  bir mevcut prop/desen yok — eklenmesi gerekecek.

**⏸️ DURAKLAMA NEDENİ — kullanıcı slug/SEO kararını SONRAYA bıraktı.**
Üçüncü soru (`/demo` vs `/landing` + noindex mi olsun) kullanıcı
tarafından şimdilik ertelendi ("buna sonra karar vereceğim... kaydet
sonra devam edeceğiz") — bu karar sayfa dosyasının nereye/hangi route'a
konacağını belirlediği için, slug netleşmeden dosya oluşturulmadı.
**Sıradaki adım (kullanıcı slug/noindex kararını verince buradan devam):**
onaylanan pilot kapsamı — yalnızca sayfa iskeleti (başlık + fayda listesi
+ form Adım 1) kurulup gösterilecek, onay sonrası Adım 2 + (ayrı
kapsamlı) backend bağlantısı tamamlanacak. Tasarım: mevcut marka
paleti (kırmızı/gri/siyah/beyaz) — kullanıcının referans görselindeki
bordo/turuncu KULLANILMAYACAK (Track B, henüz onaylanmamış, ayrı bir
karar konusu — bu ayrımın kod tabanında bir karşılığı yok, yalnızca bu
konuşmada verilmiş bir kullanıcı talimatı).

---

**2026-08-05 günlüğü (2) — Kapsamlı URL denetimi (canlı `sitemap_index.xml`
vs `astro build` çıktısı) + bulunan gerçek eksiklerin giderilmesi.**
Kullanıcı yalnızca blog değil TÜM sitenin URL yapısının canlı idenfit.com
ile karşılaştırılmasını istedi — 4 alt-sitemap (`post`/`page`/`sectors`/
`category`, toplam 853 gerçek URL) `curl` ile çekilip projenin 871 gerçek
sayfa + 1470 redirect stub'ından oluşan tam listesiyle path bazında
karşılaştırıldı. **Bulgular (rapor önce sunuldu, kullanıcı onayı sonrası
uygulandı):**
- **9 sayfa "aynı içerik, farklı slug, redirect yok"** (Müşteriler EN/IT/NL,
  Online Sunum Talebi EN/IT, Teşekkürler EN, Donanım TR/EN/IT) — TR'nin
  Online Sunum Talebi'i için zaten var olan güvenlik ağı deseni (bkz.
  `/online-sunum-talep-et` → `/online-sunum-talebi`) diğer 8 sayfaya hiç
  uygulanmamıştı. **Düzeltildi:** `astro.config.mjs`'in `redirects`
  tablosuna 9 satır eklendi.
- **2 modül sayfası TAMAMEN kaçırılmış:** Demirbaş Yönetimi Modülü
  (TR `/demirbas-yonetimi-modulu/` + EN `/en/fixed-asset-management-module/`,
  canlı title "Demirbaş Modülü") ve Seyahat ve Görevlendirme Yönetimi
  Modülü (TR `/seyahat-ve-gorevlendirme-yonetimi-modulu/` + EN
  `/en/travel-and-assignment-management-module/`, canlı title "Seyahat
  Modülü") — ikisinin de "Zimmet Yönetimi"yle karıştırılabilecek ama
  canlıda TAMAMEN AYRI, kendi title'ı olan sayfaları var. **Kök neden:**
  ikisi de `reference/wordpress-export/pages.json`'da (orijinal REST
  crawl'da) ZATEN vardı — yalnızca `extract-products.mjs`'in
  `MODULE_TR_SLUGS` listesine hiç eklenmemişlerdi, modül migrasyon
  turlarının hiçbirinde fark edilmemiş. **Düzeltildi:** iki slug listeye
  eklendi, `products.json` yeniden üretildi (Şema A, ikisi de
  `module_title` kullanıyor, ek bir extraction mantığı gerekmedi), 4 yeni
  sayfa (`demirbas-yonetimi-modulu.astro`/`en/fixed-asset-management-module.astro`/
  `seyahat-ve-gorevlendirme-yonetimi-modulu.astro`/
  `en/travel-and-assignment-management-module.astro`) `zimmet-yonetimi-modulu.astro`
  ile BİREBİR aynı desenle (mevcut `ProductPage.astro`/`productContent.ts`,
  SLUG_INDEX otomatik `link` alanından per-locale slug türetiyor, elle
  eşleme gerekmedi) kuruldu. **Kaynak veri notu (tahmin değil, ham ACF'ten
  doğrulandı):** Demirbaş'ın TR kaydında SSS (`question_N`) alanları hiç
  dolu değil (0), EN'de 4 tane dolu — kaynağın kendi asimetrisi, KARAR
  1'in kapsamına girmiyor (EN'in TR'den kopya/eksik OLMASI değil, TR'nin
  EN'de olan bir SSS bloğunu hiç içermemesi) — uydurulmadı, TR sayfası SSS
  bölümü olmadan render ediliyor (mevcut `ProductPage.astro` zaten 0
  öğeli `faq` dizisini destekliyor). Demirbaş'ın NL çevirisi de kaynakta
  var ama BU TURDA istenmedi (kullanıcı açıkça TR+EN dedi), eklenmedi.
- **KVK Protokol (TR+EN) eklendi — ana KVKK Aydınlatma Metni'nden AYRI bir
  hukuki doküman, farklı bir extraction yolu gerektirdi.** Diğer "legal"
  sayfaların (`sozlesme/tuketici-haklari` vb.) aksine bir ACF
  `content_block` alanı YOK — ham belge doğrudan `content.rendered`
  içinde bir Elementor "HTML" widget'ına (`widget_type="html.default"`)
  yapıştırılmış. **Kaynağın kendi tutarsızlığı:** TR kaydı
  `<section class="contract-body">` etiketini HİÇ KAPATMAMIŞ (well-formed
  değil, tarayıcı toleranslı olduğu için canlıda sorun çıkmıyor), EN kaydı
  kapatmış — iki dil arasında bile farklı. Yeni
  `extractLegalFromRenderedHtml()` (`extract-misc-pages.mjs`) önce gerçek
  `</section>`'i arıyor (varsa, EN gibi, en güvenilir sınır), yoksa (TR
  gibi) Elementor'un kendi wrapper `<div>`'lerinin saf/ardışık kapanış
  zincirinden yalnızca içeriğe ait SON gerçek kapanışı (`highlight` bloğu)
  geri bırakıp gerisini atıyor — iki yöntem de test edilip her iki dilde
  de div sayısının dengeli çıktığı (`<div>`/`</div>` eşit) doğrulandı.
  `group()`'a yeni bir `'legalRendered'` türü eklendi (`extractFn(entry)`
  çağırıyor, diğerleri gibi `extractFn(entry.acf)` değil) — çıktısı aynı
  `{contentHtml}` şekli olduğundan `miscPagesContent.ts`'teki
  `getLegalContent()` DEĞİŞMEDEN yeniden kullanılabildi (yeni
  `getKvkProtocolContent/-Slug/-LocaleUrls`). **Yan bulgu/düzeltme:**
  belgenin kendi başlık div'i (`div.contract-title`, sayfanın `<h1>`'iyle
  örtüşen bir tekrar) semantik bir `<h2>`'ye çevrildi; bu da
  `LegalPage.astro`'nun `[&_h1]` ARDINDAN gerçekte hiç `[&_h2]`/`[&_h3]`/
  `[&_h4]` stili OLMADIĞINI ortaya çıkardı (`misc-pages.json`'daki diğer
  legal içerikler h2/h3/h4 kullanıyor, h1 DEĞİL — muhtemelen fark
  edilmemiş bir kozmetik boşluk) — üç seviye de eklendi, tüm legal
  sayfaları (yeni + mevcut) etkiliyor, tamamen katkısal/risksiz.
- **4 sayfa BİLİNÇLİ OLARAK ATLANDI (kullanıcı kararı):** `/kampanya/`
  ("Aralık Kampanyası" — süresi geçmiş promosyon), `/slider-form/` +
  `/en/slider-form-en/` (title "Slider Form" — muhtemelen Elementor
  popup/gömme şablonu, bağımsız içerik değil), `/sample-page/` +
  `/test-tr/` (WP varsayılan/test çöpü). Migrate edilmedi, kod
  değişikliği yok.
- **Kategori arşiv sayfaları (12, `/category/*/`) bu turda ele alınmadı —
  yeni açık nokta olarak aşağıya eklendi (bkz. #22).**

**Test (her adımdan sonra ayrı ayrı koşuldu):** `astro check` her adımda 0
hata (290→292 dosya), `astro build` 870→874 (2 modül×2 dil)→876 (KVK
Protokol×2 dil) sayfa, hiçbiri hatasız, `test-no-external-idenfit-links.mjs`
her turda temiz (2350→2360→2365, redirect stub sayısı arttıkça dosya
sayısı da arttı), `test-urunler-menu-links.mjs` 108/108 (nav'a
dokunulmadı, bkz. aşağıdaki açık nokta). `dev:clean` + `curl` ile 9
redirect'in tamamı, 4 yeni modül sayfasının H1/görsel/liste içeriği, ve
KVK Protokol'ün H1/H2 başlıkları + DOM bütünlüğü (footer'ın hâlâ doğru
render edildiği, ham HTML'deki kapanmamış `<section>`'ın sızmadığı) tek
tek doğrulandı.

**Açık nokta — yeni modül sayfaları mega-menüde YOK, kullanıcıya
soruldu.** Demirbaş ve Seyahat sayfaları kuruldu ama `src/data/navigation.ts`'teki
ÜRÜNLER mega-menüsüne (108/108 link, "TAMAMEN KAPANDI" olarak
belgelenmişti) EKLENMEDİ — bu, i18n çeviri anahtarları (4 dil dosyası) +
hangi menü kolonuna gireceği kararını gerektiren ayrı bir kapsam
genişlemesi, kullanıcının bu turki talimatı yalnızca "sayfaları kur"
kapsamındaydı. Sonuç: 2 sayfa şu an yalnızca doğrudan URL/redirect ile
erişilebilir, nav'dan ulaşılamıyor ("orphan" sayfa) — kullanıcıya ayrıca
soruldu.

**2026-08-05 günlüğü — pilot Markdown dosyalarında gerçek bug bulundu:
göç script'i HTML→Markdown dönüşümü hiç yapmıyordu, düzeltildi.**
Kullanıcı Decap panelinde Rich Text editörünü açınca 4 pilot yazının
gövdesinin ham WP HTML'i (`<p class="wp-block-paragraph">`, `<strong>`,
`rel="noreferrer noopener"` vb.) olarak göründüğünü, kod gibi
kullanılamaz durumda olduğunu bildirdi. **Kök neden:**
`migrate-blog-to-markdown.mjs` `post.content`'i (posts.json'daki ham WP
HTML) hiçbir dönüşümden geçirmeden doğrudan `.md` gövdesine yazıyordu —
site tarafında bu "kazara çalışıyordu" çünkü CommonMark blok-seviyeli ham
HTML'i olduğu gibi geçiriyor (bkz. günlük (11)'deki `legacyJsonLoader`
mantığıyla aynı passthrough), ama Decap'in Rich Text editörü gerçek
Markdown sözdizimi bekliyor, ham HTML'i metin olarak gösteriyor.

**Düzeltme:** `turndown` devDependency olarak eklendi, paylaşılan
`scripts/lib/html-to-markdown.mjs` modülü yazıldı (`migrate-blog-to-markdown.mjs`
VE tek seferlik `scripts/reprocess-migrated-blog-html.mjs` tarafından
ortak kullanılıyor) — `<strong>`→`**`, `<a href>`→`[metin](url)`, `<p>`→
paragraf boşluğu, `<h2/h3>`→`##`/`###`. 3 gerçek veri kalitesi sorunu
ayrıca ele alındı: (1) WP'nin kopyala-yapıştır artığı iç içe
`<strong><strong>...</strong></strong>` dönüşümden ÖNCE tek katmana
indiriliyor (aksi halde `****metin**` gibi geçersiz Markdown üretiyordu),
(2) boş `<p><br></p>` blokları temizleniyor, (3) `<br>` artık iki
boşluk+satır sonu yerine görünür `\`+satır sonu (git/editör
whitespace-trim'ine karşı dayanıklı). `<iframe>` (Google Form embed'i)
Markdown'da temsil edilemediği için BİLİNÇLİ olarak ham HTML kalıyor
(`turndownService.keep(['iframe'])`, CommonMark'ın raw-HTML-passthrough'u
sayesinde geçerli kalıyor) — içerik kaybı yaşanmasın diye, dokümante
edilmiş tek istisna. `<mark>` (WP'nin inline renk vurgusu) Markdown'da
karşılığı olmadığı için kaldırılıp metni korunuyor (rengi kaybolur,
bilinçli bir sınır).

4 pilot dosya `reprocess-migrated-blog-html.mjs` ile yeniden işlendi
(frontmatter dokunulmadı, yalnızca gövde). **Test:** `astro check` 0
hata (286 dosya), `astro build` 870 sayfa hatasız (sayı değişmedi),
`test-no-external-idenfit-links.mjs` 2341/0, `dev:clean` sonrası `curl`
ile en karmaşık pilot yazının (`zirve-katilim-ix-...`, iç içe
`<div>`+`<iframe>` içeren) render edilen HTML'i doğrulandı: gerçek
`<strong>`(9 adet)/`<a href>`/`<p>` etiketleri üretiyor, `wp-block-*`
class'ı SIFIR, kaçış karakterli (`&lt;p class`) literal tag metni SIFIR.

**Dürüst sınır:** Chrome uzantısı bu oturumda da bağlı değildi
(`tabs_context_mcp` "not connected" döndü) — Decap panelinin GERÇEK Rich
Text görünümünün ekran görüntüsü alınamadı, yalnızca sitenin render
ettiği HTML çıktısı `curl` ile doğrulandı (dolaylı ama güçlü bir kanıt —
Decap'in markdown widget'ı standart CommonMark/markdown-it kullanıyor,
üretilen Markdown bu sözdizimine birebir uyuyor). **Kullanıcının
`localhost:4321/admin`'de 4 pilot yazıyı açıp görsel olarak doğrulaması
gerekiyor** — bkz. Açık nokta #21 (güncellendi: kademeli üretim artık bu
dönüşümü otomatik uyguluyor).
...
21. **YENİ — Blog CMS entegrasyonu (Decap CMS) devam ediyor, 3 açık uç
    ...
    - **KADEMELİ ÜRETİM bekliyor:** pilot 4 yazı göç etti, kalan 618
      ...
      **[2026-08-05] Script artık `htmlToMarkdown()` ile otomatik
      HTML→Markdown dönüşümü uyguluyor** (bkz. aynı tarihli günlük) —
      kalan 618 yazı için ayrıca bir dönüşüm adımı gerekmiyor.
...
22. **YENİ — Kategori arşiv sayfaları (`/category/<slug>/`, canlıda 12
    taksonomi sayfası) migrate edilmedi, ayrı bir yapısal karar bekliyor
    (2026-08-05, kapsamlı URL denetimi).** Blog mimarisi tek koleksiyon +
    `/blog/page/N/` sayfalama kullanıyor (bkz. §Mimari — "Blog Content
    Collection"), kategoriye göre filtrelenmiş ayrı bir arşiv sayfası
    ailesi hiç yok. Bunu eklemek 12+ yeni route (× kategori sayısı kadar,
    potansiyel çoklu dil) demek — Content Collection'da kategori bazlı
    filtreleme + sayfalama + SEO (canonical/meta) kurulumu gerektiren yeni
    bir özellik, mevcut bir sayfanın küçük bir düzeltmesi değil. Kullanıcı
    kararıyla bu turda ele alınmadı, kod değişikliği yok.
23. ~~Demirbaş Yönetimi Modülü ve Seyahat ve Görevlendirme Yönetimi
    Modülü sayfaları ÜRÜNLER mega-menüsüne eklenmedi~~ — **✅ KARAR
    VERİLDİ, bilinçli (2026-08-05):** kullanıcı "canlıda menüde var mı"
    sorusunu sordu — canlı ana sayfanın tam HTML'i kontrol edildi (Zimmet
    kontrol grubu olarak kullanıldı: `zimmet-yonetimi-modulu` linki
    sayfada 3 kez geçiyor). **Demirbaş/Seyahat canlı HTML'de SIFIR
    eşleşme** (yalnızca `<a href>` değil, sayfanın tamamında) — yani
    kaynak site de bu 2 sayfayı KASITLI OLARAK menüden gizli tutuyor,
    yalnızca sitemap/doğrudan URL ile bulunabiliyorlar. Bizim tarafımızda
    da nav'a EKLENMEDİ — kaynağın kendi davranışına sadakat. Sayfalar
    `/demirbas-yonetimi-modulu/`, `/seyahat-ve-gorevlendirme-yonetimi-modulu/`
    (+ EN karşılıkları) URL/redirect ile erişilebilir durumda kalıyor.
