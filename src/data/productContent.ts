...
const SYNTHETIC_SLUGS: Record<string, Partial<Record<Locale, string>>> = {
  // az (2026-08-21) — kaynakta hiç az verisi yok; Azerice ile Türkçenin
  // yakınlığı nedeniyle bilinçli olarak TR'nin BİREBİR AYNI bare slug'ı
  // yeniden kullanıldı (yeni bir Azerice slug icat edilmedi) — bu, sektör
  // sayfalarındaki "Bu Sektöre Özel Modüller" CTA linklerinin (TR slug'ını
  // referans alan) az'da da doğru hedefe gitmesini garanti eder.
  'anket-modulu': { az: 'anket-modulu' },
  'bordo-onay-modulu': { az: 'bordo-onay-modulu' },
  'calisan-egitim-yonetimi-modulu': { az: 'calisan-egitim-yonetimi-modulu' },
  // NOT: gerçek TR bare slug'ı "calisan-ozluk-yonetimi-modulu" (trSlug'daki
  // "-2" sadece dahili anahtar, gerçek route değil — bkz. products.json'daki
  // TR `link` alanı) — az fiziksel sayfa dosyası da aynı isimle oluşturuldu.
  'calisan-ozluk-yonetimi-modulu-2': { az: 'calisan-ozluk-yonetimi-modulu' },
  'calisan-performans-degerlendirme-sistemi-modulu': { az: 'calisan-performans-degerlendirme-sistemi-modulu' },
  'demirbas-yonetimi-modulu': { az: 'demirbas-yonetimi-modulu' },
  'dokuman-yonetim-sistemi-modulu': { az: 'dokuman-yonetim-sistemi-modulu' },
  'isyeri-saglik-yonetimi-modulu': { az: 'isyeri-saglik-yonetimi-modulu' },
  'ise-alim-modulu': { nl: 'wervingsmodule', az: 'ise-alim-modulu' },
  'masraf-yonetimi-modulu': { az: 'masraf-yonetimi-modulu' },
  'kurumsal-takvim-modulu': { az: 'kurumsal-takvim-modulu' },
  'ofis-ici-mesajlasma-modulu': { az: 'ofis-ici-mesajlasma-modulu' },
  'onboarding-modulu': { az: 'onboarding-modulu' },
  'organizasyonel-geri-bildirim-modulu': { az: 'organizasyonel-geri-bildirim-modulu' },
  'puantaj-takip-programi-modulu': { az: 'puantaj-takip-programi-modulu' },
  'offboarding-modulu': { az: 'offboarding-modulu' },
  'pdks-modulu': { az: 'pdks-modulu' },
  'personel-servisi-yonetimi-modulu': { az: 'personel-servisi-yonetimi-modulu' },
  'notlar-modulu': { az: 'notlar-modulu' },
  'zimmet-yonetimi-modulu': { az: 'zimmet-yonetimi-modulu' },
  'yillik-izin-takip-programi': { az: 'yillik-izin-takip-programi' },
  'seyahat-ve-gorevlendirme-yonetimi-modulu': { az: 'seyahat-ve-gorevlendirme-yonetimi-modulu' },
};
...
export function getProductContent(trSlug: string, locale: Locale): ProductContent | undefined {
  ...
  // çeviri ayrı `PRODUCT_OVERRIDES_AZ` tablosundan geliyor (KARAR 1).
  // Mevcut 19 trSlug'lık `PRODUCT_OVERRIDES` iç içe yapısını bozmadan
  // AYRI bir öncelik olarak kontrol ediliyor (bkz. o dosyanın yorumu).
  if (locale === 'az') {
    const azOverride = PRODUCT_OVERRIDES_AZ[trSlug];
    if (!azOverride) return undefined;
    ...
    return {
      ...
      sections: azOverride.sections.map(resolveBlock),
      faq: azOverride.faq.map((f) => ({ question: f.question, answer: cleanRichText(f.answer) })),
    };
  }
...
export function getProductPageTitle(trSlug: string, locale: Locale): string {
  if (locale === 'az') return stripTags(PRODUCT_OVERRIDES_AZ[trSlug]?.hero.title || trSlug);
  const entry = DATA.products.find((g) => g.trSlug === trSlug)?.locales[locale];
  const override = PRODUCT_OVERRIDES[trSlug]?.[locale];
